...
const DetailUser = () => {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const response = await fetch(`https://users-backend-management.onrender.com/app/users/${id}`);
                if (!response.ok) {
    ...
    const handleDelete = async (userId) => {
        ...
        if (confirmation) {
          try {
            const response = await fetch(`https://users-backend-management.onrender.com/app/users/${userId}`, {
              method: 'DELETE',
            });
            if (!response.ok) {
